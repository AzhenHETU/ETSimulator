...
$(function() {
    $( "#sliderChargingFleetSize" ).slider({
      value: 10,
      min: 0,
      max: 250,
      step: 10,
      slide: function( event, ui ) {
        $( "#chargingFleetSize" ).val( ui.value );
      }
    });
    $( "#chargingFleetSize" ).val(  $( "#sliderChargingFleetSize" ).slider( "value" )  );
});
...
function fleet_sim() {
    ...
    $.ajax({
					url: '/run/',
					method: 'POST',
          // data: JSON.stringify(sim_params),
          data: JSON.stringify(sim_params),
...
//the fleet-oriented waiting time
